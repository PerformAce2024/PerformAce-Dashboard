...
const fetchTaboolaDailyMetrics = async () => {
  try {
    ...
    // const dailyMetricsUrl = `https://backend-api.performacemedia.com:8000/api/taboola/getClientDailyMetrics/${campaignId}`;
    const dailyMetricsUrl = `${config.BASE_URL}/api/taboola/getClientDailyMetrics/${campaignId}`;
    ...
    if (data && data.dailyMetrics && data.dailyMetrics.length > 0) {
      ...
      data.dailyMetrics.forEach((metric) => {
        const amountSpent = metric.amountSpent
          ? `₹${metric.amountSpent.toFixed(2)}`
          : "N/A";
        const ctr = metric.ctr ? `${metric.ctr.toFixed(2)}%` : "N/A";
        const clientCPCDisplay = !isNaN(clientCPC)
          ? `₹${clientCPC.toFixed(2)}`
          : "N/A";
