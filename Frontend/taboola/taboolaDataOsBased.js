...
const fetchOSPerformanceData = async (campaignId) => {
  try {
    ...
    const apiUrl = `${config.BASE_URL}/api/taboola/getClicksByOS/${campaignId}`;
    const response = await fetch(apiUrl);

    if (!response.ok) {
      throw new Error(
        `Failed to fetch OS performance data: ${response.statusText}`
      );
    }
      ...
      datasets: [
        {
          ...
          borderWidth: 1,
          data: osClicks.map((clicks) =>
            (
              (clicks / osClicks.reduce((sum, clicks) => sum + clicks, 0)) *
              100
            ).toFixed(2)
