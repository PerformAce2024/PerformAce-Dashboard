...
const fetchCampaignDataAggregates = async () => {
  try {
    ...
    const campaignRequestUrl = `${config.BASE_URL}/api/taboola/getCampaignAggregates/${campaignId}`;

    console.log(
      `Requesting campaign aggregates from URL: ${campaignRequestUrl}`
    );
    ...
    const totalClicks = data.top3ClicksData.reduce(
    ...
    updatePieChart(
      clicksData[0].clicks,
      totalClicks,
      document.querySelector(".js-easy-pie-chart-1"),
      clicksData[0].state,
      ".js-state-name-1"
    );
    updatePieChart(
      clicksData[1].clicks,
      totalClicks,
      document.querySelector(".js-easy-pie-chart-2"),
      clicksData[1].state,
      ".js-state-name-2"
    );
    updatePieChart(
      clicksData[2].clicks,
      totalClicks,
      document.querySelector(".js-easy-pie-chart-3"),
      clicksData[2].state,
      ".js-state-name-3"
    );
    updatePieChart(
      clicksData[3].clicks,
      totalClicks,
      document.querySelector(".js-easy-pie-chart-4"),
      "Other States",
      ".js-state-name-4"
    );
  } catch (error) {
...
const updatePieChart = (
  ...
  console.log(
    `Updating pie chart for state: ${stateName} with clicks: ${clicks}`
  );
  ...
  if (pieChartElement) {
    ...
    if (percentElement) {
      ...
      console.log(
        `Updated pie chart percentage for ${stateName}: ${Math.round(
          clicksReceived
        )}%`
      );
    }
