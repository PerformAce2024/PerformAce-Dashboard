...
const fetchAndDisplayCampaignPerformance = async (campaignId) => {
  try {
    ...
    const apiUrl = `${config.BASE_URL}/api/taboola/getCampaignDailyData/${campaignId}`;

    const response = await fetch(apiUrl);

    if (!response.ok) {
      throw new Error(
        `Failed to fetch campaign daily data: ${response.statusText}`
      );
    }
...
const updateAreaChart = (dates, clicks, impressions) => {
  ...
  if (!canvasElement) {
    console.error(
      "Canvas element with id 'campaignAreaChart' not found in the DOM."
    );
    return;
  }
