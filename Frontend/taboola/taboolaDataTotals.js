...
const fetchCampaignDataTotal = async () => {
  try {
    ...
    const campaignRequestUrl = `${config.BASE_URL}/api/taboola/getCampaignTotals/${campaignId}`;
    ...
    console.log(
      `Total Clicks: ${totalClicks}, Total Impressions: ${totalImpressions}, Total Spent: ₹${totalSpent}, CTR: ${averageCTR}%`
    );
    ...
    if (clicksProgressBar) {
      ...
      console.log(
        "Updated clicks progress bar width:",
        `${clicksProgressPercentage}%`
      );
    } else {
    ...
    if (spentProgressBar) {
      ...
      console.log(
        "Updated spent progress bar width:",
        `${spentProgressPercentage}%`
      );
    } else {
    ...
    const impressionsProgressPercentage =
      (totalImpressions / maxImpressions) * 100;
    const impressionsProgressBar = document.querySelector(
      ".progress-bar-impressions"
    );
    if (impressionsProgressBar) {
      impressionsProgressBar.style.width = `${impressionsProgressPercentage}%`;
      console.log(
        "Updated impressions progress bar width:",
        `${impressionsProgressPercentage}%`
      );
    } else {
    ...
    if (ctrProgressBar) {
      ...
      console.log(
        "Updated CTR progress bar width:",
        `${ctrProgressPercentage}%`
      );
    } else {
...
const renderLineChart = (clicksData) => {
  const formattedClicksData = clicksData.map((item) => [
