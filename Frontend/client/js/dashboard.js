async function fetchAndPopulateROs() {
    try {
        ...
        const response = await fetch(`http://15.207.100.193:8000/api/client/ros/${userEmail}`, {
            method: 'GET',
            headers: {
...
async function fetchAndDisplayClientName() {
    try {
        ...
        const clientNameUrl = `http://15.207.100.193:8000/api/clientname/${encodeURIComponent(userEmail)}`;
