document.addEventListener('DOMContentLoaded', () => {
    const generatePDFButton = document.querySelector('.btn.btn-default.btn-block');
    ...
    generatePDFButton.addEventListener('click', async function(e) {
        ...
        try {
            ...
            const cleanData = {
                dailyMetrics: dailyMetrics.dailyMetrics?.map(({date, impressions, clicks}) => ({
                    date: new Date(date).toLocaleDateString(),
                    impressions,
                    clicks
                })),
 
                browserMetrics: browserMetrics?.map(({browser, clicks, impressions, spent, visible_impressions}) => ({
                    browser,
                    clicks,
                    impressions,
                    spent,
                ...
                regionMetrics: regionMetrics.allStatesData?.map(({state, clicks, impressions}) => ({
                    state,
                    clicks,
                    impressions
                })),
 
                siteMetrics: siteMetrics?.map(({site_name, clicks, impressions, visible_impressions, spent, ctr}) => ({
                    site_name,
                    clicks,
                    impressions, 
                    visible_impressions,
                ...
                osMetrics: osMetrics?.map(({os_family, clicks, impressions, spent, visible_impressions}) => ({
                    os_family,
                    clicks,
                    impressions,
                    spent,
            ...
            Object.entries(cleanData).forEach(([name, data]) => {
                ...
                XLSX.utils.book_append_sheet(workbook, sheet, name.replace('Metrics', ' Metrics'));
            });
