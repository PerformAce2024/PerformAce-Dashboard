...
const fetchBrowserStatistics = async (roNumber) => {
  try {
    ...
    const apiUrl = `${config.BASE_URL}/api/metrics/browser?clientEmail=${email}&roNumber=${roNumber}&startDate=&endDate=`;
...
const updateBrowserPieChart = (labels, data) => {
  ...
  window.browserChart = new Chart(canvas.getContext("2d"), {
    ...
    data: {
      datasets: [
        {
          data: data.map((clicks) =>
            ((clicks / data.reduce((a, b) => a + b, 0)) * 100).toFixed(2)
          ),
          backgroundColor: [
            "rgba(75, 192, 192, 0.6)",
