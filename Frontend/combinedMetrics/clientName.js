...
const fetchAndDisplayUserInfo = async () => {
  try {
    ...
    if (data.success) {
      ...
      const logoutSpan = document.querySelector(
        'a[href="../page_login.html"] .float-right.fw-n'
      );
      if (logoutSpan) {
      ...
      document.querySelector(
        'a[data-action="app-fullscreen"] .float-right'
      ).textContent = "F11";
      document.querySelector(
        'a[data-action="app-print"] .float-right'
      ).textContent = "Ctrl + P";
    }
  } catch (error) {
