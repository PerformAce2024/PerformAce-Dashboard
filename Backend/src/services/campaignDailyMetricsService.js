...
async function processRoCampaigns(db, campaigns, ro, clientData) {
    ...
    for (let date = new Date(dateRange.startDate); date <= new Date(dateRange.endDate); date.setDate(date.getDate() + 1)) {
...
for (const doc of platformData.taboola) {
    ...
    if (formattedDate === dateStr) {
        ...
        console.log(`Campaign ${doc.campaignId}: clicks=${clicks}, impressions=${impressions}, spent=${spent}`);
...
if (taboolaTotalClicks > 0 || taboolaTotalImpressions > 0) {
    ...
    console.log(`Taboola totals for ${dateStr}: clicks=${taboolaTotalClicks}, impressions=${taboolaTotalImpressions}, spent=${taboolaTotalSpent}`);
}
        ...
        for (const doc of platformData.outbrain) {
            ...
            if (docDate === dateStr) {
                ...
                metrics.platformData.outbrain.impressions += getNumber(result.impressions);
                metrics.platformData.outbrain.spent += getNumber(result.spend || result.spent);
            }
        }
        ...
        for (const doc of platformData.dv360) {
            ...
            if (docDate === dateStr) {
                ...
                metrics.platformData.dv360.impressions += getNumber(result.total_impressions);
        ...
        Object.entries(metrics.platformData).forEach(([platform, data]) => {
            console.log(`${platform}: ${data.clicks} clicks, ${data.impressions} impressions, ${data.spent} spent`);
        });
        console.log(`Total: ${metrics.clicks} clicks, ${metrics.impressions} impressions, ${metrics.spent} spent`);
    }

    // Process dimension metrics
    processTaboolaSitePerformance(dimensionMetrics.performanceBySite, platformData.taboola);
    processMgidSitePerformance(dimensionMetrics.performanceBySite, platformData.mgid);
        ...
        if (doc?.performanceByDomain) {
            ...
            Object.entries(doc.performanceByDomain).forEach(([date, sites]) => {
                ...
                sites.forEach(siteData => {
                    ...
                    const newSpent = getNumber(siteData['Spent, INR']?.replace(/[^0-9.]/g, ''));
...
function processDimensionMetrics(dimensionMetrics, platform, documents) {
    documents.forEach(doc => {
        ...
        if (doc.performanceByBrowser) {
            processBrowserMetrics(dimensionMetrics.performanceByBrowser, doc.performanceByBrowser, platform);
        }

        // Process OS metrics
        if (doc.performanceByOS) {
            processOSMetrics(dimensionMetrics.performanceByOS, doc.performanceByOS, platform);
        }

        // Process Region metrics
        if (doc.performanceByRegion) {
            processRegionMetrics(dimensionMetrics.performanceByRegion, doc.performanceByRegion, platform);
        }
    });
}
...
function processOSMetrics(osMetrics, data, platform) {
    const metrics = platform === 'mgid' ? 
        ...
        Array.isArray(data) ? data.filter(Boolean) : Object.values(data).flat().filter(Boolean);
    ...
    metrics.forEach(metric => {
        ...
        osMetrics[osName].visible_impressions += getNumber(metric.visible_impressions || 0);
    });
}
...
function processRegionMetrics(regionMetrics, data, platform) {
        ...
        case 'mgid':
            Object.values(data).forEach(dateData => {
                if (Array.isArray(dateData)) {
                    metrics.push(...dateData.map(item => ({
    ...
    metrics.forEach(metric => {
        ...
        if (!regionMetrics[regionName].platform_data[metric.platform]) {
            ...
            regionMetrics[regionName].platform_data[metric.platform].clicks += metric.clicks;
            regionMetrics[regionName].platform_data[metric.platform].impressions += metric.impressions;
            regionMetrics[regionName].platform_data[metric.platform].spent += metric.spent;
        }
    });

    // Calculate metrics for each region
    Object.values(regionMetrics).forEach(region => {
        region.ctr = region.impressions > 0 ? (region.clicks / region.impressions) * 100 : 0;
...
function processMgidData(mgid, dateStr) {
    ...
    for (const doc of mgid) {
        if (doc?.campaignPerformanceResult?.results?.[1]) { // Note: Using index 1 for MGID
            const result = doc.campaignPerformanceResult.results[1];
            if (!result.date) continue;
...
function formatDimensionMetrics(metrics) {
    return {
        ...
        performanceByRegion: Object.values(metrics.performanceByRegion)
            .filter(m => m.clicks > 0 || m.impressions > 0),
        performanceByCountry: [],
...
function processBrowserMetrics(browserMetrics, data, platform) {
    ...
    metrics.forEach(metric => {
        ...
        browserMetrics[browserName].clicks += getNumber(metric.clicks || metric.Clicks);
        browserMetrics[browserName].impressions += getNumber(metric.impressions);
        browserMetrics[browserName].spent += getNumber(metric.spent || metric['Spent, INR']);
        browserMetrics[browserName].visible_impressions += getNumber(metric.visible_impressions || 0);
    });
}
...
async function testWithRealData() {
    ...
    try {
        ...
        for (const ro of results) {
            ...
            const campaigns = await client.db('campaignAnalytics')
            ...
            const platformData = await getAllPlatformData(client.db('campaignAnalytics'), campaigns);
...
async function getAllPlatformData(db, campaigns) {
    const campaignIds = campaigns.map(c => c.campaignId);
    
    const [taboola, outbrain, mgid, dv360] = await Promise.all([
        db.collection('taboolaData').find({ campaignId: { $in: campaignIds } }).toArray(),
        db.collection('outbrainNewDataFormat').find({ campaignId: { $in: campaignIds } }).toArray(),
        db.collection('mgid_transformed_data_final').find({ campaignId: { $in: campaignIds } }).toArray(),
        db.collection('dv360Data').find({ campaignId: { $in: campaignIds } }).toArray()
    ]);
    ...
    for (const dayMetrics of storedMetrics) {
        ...
        for (const [platform, metrics] of Object.entries(dayMetrics.platformData)) {
            ...
            if (Math.abs(metrics.clicks - expectedMetrics.clicks) > 0) {
                console.error(`❌ Clicks mismatch for ${platform}: stored=${metrics.clicks}, expected=${expectedMetrics.clicks}`);
            }
            if (Math.abs(metrics.impressions - expectedMetrics.impressions) > 0) {
                console.error(`❌ Impressions mismatch for ${platform}: stored=${metrics.impressions}, expected=${expectedMetrics.impressions}`);
            }
            if (Math.abs(metrics.spent - expectedMetrics.spent) > 1) {
                console.error(`❌ Spent mismatch for ${platform}: stored=${metrics.spent}, expected=${expectedMetrics.spent}`);
            }
        }
        ...
        const expectedTotal = {
            ...
            impressions: Object.values(expected).reduce((sum, m) => sum + m.impressions, 0),
            spent: Object.values(expected).reduce((sum, m) => sum + m.spent, 0)
        };
        
        console.log('\nTotal Metrics Comparison:');
        console.log('Stored:', { clicks: dayMetrics.clicks, impressions: dayMetrics.impressions, spent: dayMetrics.spent });
        console.log('Expected:', expectedTotal);
        
        if (Math.abs(dayMetrics.clicks - expectedTotal.clicks) > 0) {
            console.error(`❌ Total clicks mismatch: stored=${dayMetrics.clicks}, expected=${expectedTotal.clicks}`);
        }
        if (Math.abs(dayMetrics.impressions - expectedTotal.impressions) > 0) {
            console.error(`❌ Total impressions mismatch: stored=${dayMetrics.impressions}, expected=${expectedTotal.impressions}`);
        }
        if (Math.abs(dayMetrics.spent - expectedTotal.spent) > 1) {
            console.error(`❌ Total spent mismatch: stored=${dayMetrics.spent}, expected=${expectedTotal.spent}`);
        }
    }
}
...
async function testSitePerformance(storedSiteMetrics, platformData) {
    ...
    for (const doc of platformData.mgid) {
        ...
        Object.values(doc.performanceByDomain).forEach(sites => {
            sites.forEach(site => {
                const siteName = SITE_MAPPING[site.site || site._id?.$oid] || (site.site || site._id?.$oid || "Unknown");
                if (!expectedSiteMetrics[siteName]) {
                    expectedSiteMetrics[siteName] = {
                        clicks: 0,
                        impressions: 0,
                ...
                expectedSiteMetrics[siteName].clicks += getNumber(site.Clicks);
                expectedSiteMetrics[siteName].spent += getNumber(site['Spent, INR']?.replace(/[^0-9.]/g, ''));
    ...
    for (const [siteName, storedMetrics] of Object.entries(storedSiteMetrics)) {
        ...
        const expectedMetrics = expectedSiteMetrics[siteName] || { clicks: 0, impressions: 0, spent: 0, platforms: new Set() };
        
        console.log('Stored:', storedMetrics);
        console.log('Expected:', expectedMetrics);
        
        if (Math.abs(storedMetrics.clicks - expectedMetrics.clicks) > 0) {
            console.error(`❌ Site clicks mismatch for ${siteName}: stored=${storedMetrics.clicks}, expected=${expectedMetrics.clicks}`);
        }
        if (Math.abs(storedMetrics.impressions - expectedMetrics.impressions) > 0) {
            console.error(`❌ Site impressions mismatch for ${siteName}: stored=${storedMetrics.impressions}, expected=${expectedMetrics.impressions}`);
        }
        if (Math.abs(storedMetrics.spent - expectedMetrics.spent) > 1) {
            console.error(`❌ Site spent mismatch for ${siteName}: stored=${storedMetrics.spent}, expected=${expectedMetrics.spent}`);
        }
    }
}
