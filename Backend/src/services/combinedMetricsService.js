...
export const getCombinedMetrics = async (campaignId) => {
    try {
        ...
        const combinedData = {
            ...
            totalImpressions: (taboolaData.totalImpressions || 0) + (mgidData.totalImpressions || 0),
            totalSpent: (taboolaData.totalSpent || 0) + (mgidData.totalSpent || 0),
            averageCTR: ((taboolaData.totalClicks + mgidData.totalClicks) / (taboolaData.totalImpressions + mgidData.totalImpressions)) * 100 || 0,
            clicksData: [...(taboolaData.clicksData || []), ...(mgidData.clicksData || [])]
        };
