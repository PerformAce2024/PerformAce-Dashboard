...
const aggregateCampaignMetrics = (allCampaigns) => {
    ...
    allCampaigns.forEach(campaign => {
        const taboolaMetrics = campaign.rawData.taboola?.campaignPerformanceResult?.results?.[0];
        const outbrainMetrics = campaign.rawData.outbrain?.data?.campaignPerformance?.results?.[0]?.metrics;

        if (taboolaMetrics) {
            totalMetrics.clicks += getNumber(taboolaMetrics.clicks);
            totalMetrics.impressions += getNumber(taboolaMetrics.impressions);
            totalMetrics.visible_impressions += getNumber(taboolaMetrics.visible_impressions);
    ...
    const derivedMetrics = {
        ...
        vctr: totalMetrics.visible_impressions > 0 ? (totalMetrics.clicks / totalMetrics.visible_impressions) * 100 : 0,
        cpm: totalMetrics.impressions > 0 ? (totalMetrics.spent / totalMetrics.impressions) * 1000 : 0,
        vcpm: totalMetrics.visible_impressions > 0 ? (totalMetrics.spent / totalMetrics.visible_impressions) * 1000 : 0,
    ...
    return {
        last_used_rawdata_update_time: new Date().toLocaleString('en-US', { timeZone: 'Asia/Kolkata', hour12: false }),
...
const aggregateDimensionMetrics = (allCampaigns, dimensionType) => {
    ...
    allCampaigns.forEach(campaign => {
        ...
        const taboolaData = campaign.rawData.taboola?.[`performanceBy${dimensionType}`]?.results || [];
        taboolaData.forEach(item => {
            const key = dimensionType === 'Region' ? item.region : item[dimensionType.toLowerCase()] || 'Unknown';
        ...
        const outbrainResults = campaign.rawData.outbrain?.data?.[`${dimensionType.toLowerCase()}Performance`]?.results || [];
        outbrainResults.forEach(result => {
            const key = dimensionType === 'Region' ? 
...
async function aggregateClientData(clientEmail, startDate, endDate) {
    ...
    try {
        ...
        await db.collection('aggregatedTableFromAllPlatforms').updateOne(
