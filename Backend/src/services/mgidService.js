...
export async function transformMgidData(campaignId, startDate, endDate) {
    ...
    try {
        ...
        Object.keys(transformedData.performanceByRegion).forEach(dateKey => {
            ...
            transformedData.performanceByRegion[dateKey] = aggregateRegionDataByState(regionData);
        });

        const transformedCollection = client.db('campaignAnalytics').collection('mgid_transformed_data_final');
