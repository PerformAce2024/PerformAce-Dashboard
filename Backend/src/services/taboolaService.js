...
export const getTaboolaPerformanceBySite = async (campaignId, startDate, endDate) => {
    const requestUrl = `${TABOOLA_BASE_URL}${TABOOLA_ACCOUNT_ID}/reports/campaign-summary/dimensions/site_breakdown?start_date=${startDate}&end_date=${endDate}${campaignId ? `&campaign=${campaignId}` : ''}&include_multi_conversions=true&exclude_empty_campaigns=true`;
