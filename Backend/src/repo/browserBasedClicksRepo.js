...
class CampaignPerformanceByBrowserRepo {
  static async getClicksByBrowser(campaignId) {
    console.log(
      "Starting to fetch browser performance data for campaignId:",
      campaignId
    );
    ...
    const clicksAndImpressionsByBrowser = browserResults.map(
      (browser, index) => {
        ...
        const impressions =
          browser.impressions !== undefined ? browser.impressions : 0; // Default to 0 if impressions is missing

        // Log the final values being processed
        console.log(
          `Browser: ${browserName}, Clicks: ${clicks}, Impressions: ${impressions}`
        );
    ...
    console.log(
      "Final Clicks and Impressions by Browser:",
      clicksAndImpressionsByBrowser
    );
