...
class MetricsRepository {
    ...
    static async getSiteStats({ clientEmail, roNumber, startDate, endDate, limit = 10 }) {
        ...
        const sitesWithCTR = metrics.performanceBySite.map(site => ({
            ...site,
            ctr: site.impressions > 0 ? (site.clicks / site.impressions * 100).toFixed(2) : 0
        }));
    ...
    static async getRegionStats({ clientEmail, roNumber, startDate, endDate, limit }) {
        ...
        const regionsWithCTR = metrics.performanceByRegion.map(region => ({
            ...region,
            ctr: region.impressions > 0 ? (region.clicks / region.impressions * 100).toFixed(2) : 0
        }));

        const sortedRegions = [...regionsWithCTR].sort((a, b) => b.clicks - a.clicks);
        ...
        const totalImpressions = sortedRegions.reduce((sum, r) => sum + r.impressions, 0);
        const totalCTR = totalImpressions > 0 ? (totalClicks / totalImpressions * 100).toFixed(2) : 0;
