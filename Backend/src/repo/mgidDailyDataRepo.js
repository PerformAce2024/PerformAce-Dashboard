...
class MgidDailyDataRepo {
    ...
    static async getMgidCampaignDailyData(campaignId) {
        ...
        console.log(`Fetching daily performance data for MGID campaignId: ${campaignId}`);
        ...
        let dailyData = results.map(result => ({
            ...
            impressions: parseInt(result['                         Imps                          ']?.trim() || 0, 10) // Daily impressions
        }));

        // Filter out entries where the date is empty or clicks are 0
        dailyData = dailyData.filter(entry => entry.date && entry.clicks > 0 && entry.impressions);
