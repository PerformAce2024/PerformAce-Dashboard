...
class CombinedBrowserBasedClicksRepo {
    static async getCombinedClicksByBrowser(taboolaCampaignId) {
        ...
        const taboolaBrowserClicks = await CampaignPerformanceByBrowserRepo.getClicksByBrowser(taboolaCampaignId);
        console.log('Taboola Browser Clicks Data:', taboolaBrowserClicks);

        console.log('Fetching MGID browser-based clicks data...');
        const mgidBrowserClicks = await MgidPerformanceByBrowserRepo.getMgidClicksByBrowser(mgidCampaignId);
        ...
        const mergeBrowserClicksData = (data) => {
            data.forEach(entry => {
                ...
                if (combinedBrowserClicksMap.has(browser)) {
                    combinedBrowserClicksMap.set(browser, combinedBrowserClicksMap.get(browser) + entry.clicks);
                } else {
        ...
        const combinedBrowserClicksData = Array.from(combinedBrowserClicksMap, ([browser, clicks]) => ({ browser, clicks }));
