...
class CombinedTotalRepo {
    static async getCombinedCampaignPerformanceTotals(taboolaCampaignId) {
        ...
        const taboolaData = await CampaignTotalRepo.getCampaignPerformanceTotals(taboolaCampaignId);
        console.log('Taboola Data:', taboolaData);

        console.log('Fetching MGID data...');
        const mgidData = await MgidTotalRepo.getMgidCampaignPerformanceTotals(mgidCampaignId);
