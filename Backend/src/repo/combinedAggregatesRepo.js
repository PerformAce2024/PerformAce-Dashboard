...
class CombinedAggregatesRepo {
    static async getCombinedCampaignRegionAggregates(taboolaCampaignId) {
        ...
        const taboolaAggregates = await CampaignAggregatesRepo.getCampaignRegionAggregates(taboolaCampaignId);
        console.log('Taboola Aggregates:', taboolaAggregates);

        console.log('Fetching MGID aggregates...');
        const mgidAggregates = await MgidAggregatesRepo.getMgidCampaignRegionAggregates(mgidCampaignId);
        console.log('MGID Aggregates:', mgidAggregates);

        // Combine metrics
        const combinedTotals = {
            totalClicks: (mgidAggregates.totalClicks || 0) + (taboolaAggregates.totalClicks || 0),
        };
        ...
        const mergeClicksData = (data) => {
            data.forEach(entry => {
                if (combinedClicksDataMap.has(entry.region)) {
                    combinedClicksDataMap.set(entry.region, combinedClicksDataMap.get(entry.region) + entry.clicks);
                } else {
        ...
        const combinedClicksData = Array.from(combinedClicksDataMap, ([region, clicks]) => ({ region, clicks }));
        ...
        const otherClicks = combinedClicksData.slice(3).reduce((acc, curr) => acc + curr.clicks, 0);
