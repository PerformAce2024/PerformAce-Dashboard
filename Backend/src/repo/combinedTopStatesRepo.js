...
class CombinedTopStatesRepo {
    static async getCombinedTop7StatesByClicks(taboolaCampaignId) {
        ...
        const taboolaTopStates = await CampaignTopStates.getTop7StatesByClicks(taboolaCampaignId);
        console.log('Taboola Top States:', taboolaTopStates);

        console.log('Fetching MGID top states...');
        const mgidTopStates = await MgidTopStates.getMgidTop7StatesByClicks(mgidCampaignId);
        console.log('MGID Top States:', mgidTopStates);

        // Combine the total clicks from both platforms
        const combinedTotals = {
            totalClicks: (mgidTopStates.totalClicks || 0) + (taboolaTopStates.totalClicks || 0),
        };
        ...
        const mergeClicksData = (data) => {
            data.forEach(entry => {
                if (combinedClicksDataMap.has(entry.state)) {
                    combinedClicksDataMap.set(entry.state, combinedClicksDataMap.get(entry.state) + entry.clicks);
                } else {
        ...
        const combinedClicksData = Array.from(combinedClicksDataMap, ([state, clicks]) => ({ state, clicks }));
        ...
        const otherClicks = combinedClicksData.slice(7).reduce((acc, curr) => acc + curr.clicks, 0);
