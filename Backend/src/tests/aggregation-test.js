...
import { storeDailyMetricsForClient, getClientDailyMetrics } from '../services/campaignDailyMetricsService.js';
...
async function testClientDailyMetrics() {
    try {
        ...
        const clientExists = await db.collection('clients').findOne({ email: clientEmail });
        if (!clientExists) {
        ...
        // Store metrics
        const storedData = await storeDailyMetricsForClient(clientEmail, startDate, endDate);
        console.log('Stored data:', JSON.stringify(storedData, null, 2));

        // Verify structure
        if (!storedData.dailyMetrics || !Array.isArray(storedData.dailyMetrics)) {
            throw new Error('Invalid data structure: dailyMetrics missing or not an array');
        }

        // Retrieve and compare
        const retrievedData = await getClientDailyMetrics(clientEmail, startDate, endDate);
