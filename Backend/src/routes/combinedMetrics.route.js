...
router.get('/combined/getCampaignTotals/:campaignId', async (req, res) => {
    try {
        ...
        try {
            console.log('Fetching combined total campaign performance for campaignId:', campaignId);
            const combinedData = await CombinedTotalRepo.getCombinedCampaignPerformanceTotals(campaignId);
        ...
        res.status(500).json({ message: 'Error fetching combined metrics', error: error.message });
    }
});
...
router.get('/combined/getCampaignAggregates/:campaignId', async (req, res) => {
    ...
    try {
        console.log('Fetching combined campaign aggregates for campaignId:', campaignId);
        const combinedAggregates = await CombinedAggregatesRepo.getCombinedCampaignRegionAggregates(campaignId);
...
router.get('/combined/getTop7States/:campaignId', async (req, res) => {
    ...
    try {
        console.log(`Fetching combined top 7 states by clicks for campaignId: ${campaignId}`);
        const top7StatesData = await CombinedTopStates.getCombinedTop7StatesByClicks(campaignId);  // Using the new repo function
...
router.get('/combined/getCampaignDetailsNativeHub/:campaignId', async (req, res) => {
    const { campaignId } = req.params;
    console.log('GET /combined/getCampaignDetailsNativeHub/:campaignId route hit');
    console.log('Request Params:', req.params);

    try {
        console.log('Fetching combined campaign performance for nativeHub - campaignId:', campaignId);
        const combinedData = await CombinedNativeHubRepo.getCombinedCampaignPerformanceNativeHub(campaignId);
        ...
        res.status(500).send('An error occurred while fetching campaign totals for nativehub.');
    }
});
...
router.get('/combined/getCampaignDailyData/:campaignId', async (req, res) => {
    ...
    try {
        console.log('Fetching combined daily clicks and impressions for campaignId:', campaignId);
        const campaignDailyData = await CombinedDailyDataRepo.getCombinedDailyData(campaignId);
...
router.get('/combined/getClicksByOS/:campaignId', async (req, res) => {
    ...
    try {
        ...
        const clicksByOSData = await CombinedOsBasedClicksRepo.getCombinedClicksByOS(campaignId);
...
router.get('/combined/getClicksByBrowser/:campaignId', async (req, res) => {
    ...
    try {
        console.log(`Fetching combined clicks by browser for campaignId: ${campaignId}`);
        const clicksByBrowser = await CombinedBrowserBasedClicksRepo.getCombinedClicksByBrowser(campaignId);
